...
class thermostat extends eqLogic {
	...
	public static function pull($_options) {
		...
		if (isset($_options['stop']) && $_options['stop'] == 1) {
			...
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Thermostat::pull => mode smart : ', __FILE__) . print_r($_options, true));
			$cron = cron::byClassAndFunction('thermostat', 'pull', $_options);
			if (is_object($cron)) {
				$cron->remove(false);
			}
			...
			if ($thermostat->getConfiguration('smart_start') == 1) {
				log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Next info : ', __FILE__) . print_r($_options['next'], true));
				if ($_options['next']['type'] == 'thermostat') {
					log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Type thermostat envoi de la consigne : ', __FILE__) . $_options['next']['consigne']);
					...
					if (is_object($mode)) {
						log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Type mode envoi de la commande : ', __FILE__) . $_options['next']['cmd']);
						$mode->execCmd();
					}
				}
			}
	...
	public static function hysteresis($_options) {
		...
		log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Lancement du calcul d\'hystérésis', __FILE__));
		$status = $thermostat->getCmd(null, 'status')->execCmd();
		if ($status == __('Suspendu', __FILE__)) {
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Thermostat suspendu je ne fais rien', __FILE__));
			return;
		}
		if ($thermostat->getCmd(null, 'mode')->execCmd() == __('Off', __FILE__)) {
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Thermostat arrêté je ne fais rien', __FILE__));
		...
		log::add('thermostat', 'debug', $thermostat->getHumanName() . ' Calcul => consigne : ' . $consigne . ' hysteresis_low : ' . $hysteresis_low . ' hysteresis_hight : ' . $hysteresis_hight . ' temp : ' . $temp . ' etat precedent : ' . $thermostat->getCache('lastState'));
		$action = 'none';
		...
		if ($action == 'heat') {
			if ($status != __('Chauffage', __FILE__)) {
				log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Je dois chauffer', __FILE__));
				$thermostat->heat();
			}
		} elseif ($action == 'cool') {
			if ($status != __('Climatisation', __FILE__)) {
				log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Je dois refroidir', __FILE__));
				$thermostat->cool();
			}
		} elseif ($action == 'stop') {
			if ($status != __('Arrêté', __FILE__)) {
				log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Je m\'arrête', __FILE__));
				$thermostat->stopThermostat();
			}
		}
	}
	...
	public static function temporal($_options) {
		...
		log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Début calcul temporel', __FILE__));
		$thermostat->reschedule(date('Y-m-d H:i:00', strtotime('+' . $thermostat->getConfiguration('cycle') . ' min ' . date('Y-m-d H:i:00'))));
		log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Reprogrammation automatique : ', __FILE__) . date('Y-m-d H:i:s', strtotime('+' . $thermostat->getConfiguration('cycle') . ' min ' . date('Y-m-d H:i:00'))));
		$status = $thermostat->getCmd(null, 'status')->execCmd();
		if ($status == __('Suspendu', __FILE__)) {
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Thermostat suspendu', __FILE__));
			return;
		}
		if ($thermostat->getConfiguration('smart_start') == 1) {
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Programmation Smartstart', __FILE__));
			$thermostat->getNextState();
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Arrêt Smartstart', __FILE__));
		}
		$mode = $thermostat->getCmd(null, 'mode')->execCmd();
		if ($mode == 'Off') {
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Thermostat sur off', __FILE__));
			if ($status != __('Arrêté', __FILE__)) {
				$thermostat->stopThermostat();
			}
			return;
		...
		if ($cmd->getCollectDate() != '' && $cmd->getCollectDate() < date('Y-m-d H:i:s', strtotime('-' . $thermostat->getConfiguration('maxTimeUpdateTemp') . ' minutes' . date('Y-m-d H:i:s')))) {
			...
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Je ne fais rien car il n\'y a pas eu de mise a jour de la température depuis plus de ', __FILE__) . $thermostat->getConfiguration('maxTimeUpdateTemp').'min');
			$thermostat->setCache('temp_threshold', 1);
			return;
		}
		...
		if (!is_numeric($temp_in)) {
			...
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Je ne fais rien car la température intérieure n\'est pas un numérique', __FILE__));
			$thermostat->setCache('temp_threshold', 1);
			return;
		}
		$thermostat->setCache('temp_threshold', 0);
		...
		($temp_in > ($thermostat->getCache('lastOrder') + $thermostat->getConfiguration('offsetColdFaillure', 1)) && $temp_in > $thermostat->getCache('lastTempIn') && $thermostat->getCache('lastState') == 'cool' && $thermostat->getConfiguration('coeff_indoor_cool_autolearn') > 25)) {
			...
			if ($thermostat->getCache('nbConsecutiveFaillure', 0) == 2) {
				log::add('thermostat', 'error', $thermostat->getHumanName() . __(' Attention une défaillance du chauffage est détectée', __FILE__));
		...
		if ($thermostat->getCache('nbConsecutiveFaillure', 0) < 3 && $thermostat->getConfiguration('autolearn') == 1 && strtotime($thermostat->getConfiguration('endDate')) < strtotime('now')) {
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Démarre auto-apprentissage', __FILE__));
		...
		if($delta > 0){
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Delta consigne > 0 ('. $delta .'), je lance le calcul avec consigne - delta/2', __FILE__));
			$delta = $delta / 2;
		}
		$consigne = $thermostat->getCmd(null, 'order')->execCmd();
		$temporal_data = $thermostat->calculTemporalData($consigne - $delta);
		if($temporal_data['power'] > 0 && $delta > 0){
			log::add('thermostat', 'debug', $thermostat->getHumanName() . ' Power > 0 et delta consigne > 0 ('. $delta .'), je relance le calcul avec consigne + delta/2');
			$temporal_data = $thermostat->calculTemporalData($consigne + $delta);
		}
		...
		log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Durée du cycle : ' . $duration, __FILE__));
		if (($thermostat->getConfiguration('stove_boiler') == 0 && $temporal_data['power'] < $thermostat->getConfiguration('minCycleDuration', 5)) || (($thermostat->getCache('lastState') == 'heat' && $temporal_data['power'] < 1) || ($thermostat->getCache('lastState') != 'heat' && $temporal_data['power'] < $thermostat->getConfiguration('minCycleDuration', 5)))) {
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Durée du cycle trop courte, aucun lancement', __FILE__));
		...
		if ($duration >= $cycle) {
			$thermostat->reschedule(null, false);
		}
	...
	public static function window($_option) {
		...
		if (is_object($thermostat) && $thermostat->getIsEnable() == 1) {
			log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Détection d\'un changement sur une fenêtre', __FILE__));
			...
			foreach ($windows as $window) {
				if ('#' . $_option['event_id'] . '#' == $window['cmd']) {
					...
					log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Fenêtre trouvée : ' . cmd::byString($window['cmd'])->getHumanName() . ' - valeur : ' . $_option['value'], __FILE__));
					if ($_option['value'] == 0) {
						log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Fenêtre fermée', __FILE__));
						$thermostat->windowClose($window);
					} else {
						log::add('thermostat', 'debug', $thermostat->getHumanName() . __(' Fenêtre ouverte', __FILE__));
	...
	public function windowClose($_window) {
		if ($this->getCache('window::state::' . str_replace('#', '', $_window['cmd']), 0) != 1) {
			log::add('thermostat', 'debug', $this->getHumanName() . __('[windowClose] Je n\'ai jamais vu cette fenêtre ouverte, je ne fais rien', __FILE__));
			return;
		}
		...
		if ($this->getCmd(null, 'status')->execCmd() != __('Suspendu', __FILE__)) {
			log::add('thermostat', 'debug', $this->getHumanName() . __('[windowClose] Thermostat non suspendu je ne fais rien', __FILE__));
			return;
		}
		...
		if (is_numeric($restartTime) && $restartTime > 0) {
			log::add('thermostat', 'debug', $this->getHumanName() . __('[windowClose] Pause de ' . $restartTime . 's', __FILE__));
			sleep($restartTime);
		}
		...
		foreach ($windows as $window) {
			...
			if ($value == 1) {
				log::add('thermostat', 'debug', $this->getHumanName() . __('[windowClose] Fenêtre ouverte, je ne fais rien : ' . $window['cmd'], __FILE__));
				return;
			}
			...
			if ((strtotime($this->getCache('window::close::' . $cmd->getId() . '::datetime')) + $restartTime - 1) > strtotime('now')) {
				log::add('thermostat', 'debug', $this->getHumanName() . __('[windowClose] Fenêtre fermée depuis trop peu de temps, je ne fais rien : ' . $window['cmd'] . ' => ' . $this->getCache('window::close::' . $cmd->getId() . '::datetime') . '+' . $restartTime . 's', __FILE__));
				return;
			}
		}
		log::add('thermostat', 'debug', $this->getHumanName() . __('[windowClose] Toutes les fenêtres sont fermées, je relance le chauffage', __FILE__));
	...
	public function windowOpen($_window) {
		...
		if ($this->getCmd(null, 'mode')->execCmd() == __('Off', __FILE__) || $this->getCmd(null, 'status')->execCmd() == __('Suspendu', __FILE__)) {
			log::add('thermostat', 'debug', $this->getHumanName() . __('[windowOpen] Thermostat arreté ou suspendu je ne fais rien', __FILE__));
			return;
		}
		$stopTime = (isset($_window['stopTime']) && $_window['stopTime'] != '') ? $_window['stopTime'] : 0;
		if (is_numeric($stopTime) && $stopTime > 0) {
			log::add('thermostat', 'debug', $this->getHumanName() . __('[windowOpen] Pause de ' . $stopTime . 'minutes', __FILE__));
			sleep($stopTime * 60);
		}
		$cmd = cmd::byId(str_replace('#', '', $_window['cmd']));
		if (!is_object($cmd)) {
			log::add('thermostat', 'debug', $this->getHumanName() . __('[windowOpen] Commande introuvable je ne fais rien', __FILE__));
			return;
		}
		$value = $cmd->execCmd();
		...
		log::add('thermostat', 'debug', $this->getHumanName() . __('[windowOpen] Valeur commande : ' . $value, __FILE__));
		if ($value == 1) {
			log::add('thermostat', 'debug', $this->getHumanName() . __('[windowOpen] Arrêt du thermostat', __FILE__));
	...
	public function calculTemporalData($_consigne, $_allowOverfull = false) {
		...
		if (!is_numeric($temp_out)) {
			log::add('thermostat', 'debug', $this->getHumanName() . __(' Attention température extérieure erronée : ' . $temp_out, __FILE__));
			$temp_out = $_consigne;
		}

		log::add('thermostat', 'debug', $this->getHumanName() . __(' Température intérieure : ', __FILE__) . $temp_in . __(' - Température extérieure : ', __FILE__) . $temp_out . __(' - Consigne : ', __FILE__) . $_consigne);
		...
		log::add('thermostat', 'debug', $this->getHumanName() . __(' Direction : ', __FILE__) . $direction);
		if ($temp_in >= ($_consigne + 1.5) && $direction == 1) {
			if ($this->getCache('temp_threshold', 0) == 0) {
				log::add('thermostat', 'debug', $this->getHumanName() . __(' La température est supérieure à la consigne de plus de 1.5°C, je ne fais rien', __FILE__));
			}
		...
		if ($temp_in <= ($_consigne - 1.5) && $direction == -1) {
			if ($this->getCache('temp_threshold', 0) == 0) {
				log::add('thermostat', 'debug', $this->getHumanName() . __(' La température est inférieure à la consigne de plus de 1.5°C, je ne fais rien', __FILE__));
			}
			$this->setCache('temp_threshold', 1);
			return array('power' => 0, 'direction' => $direction);
		}
		...
		if(!$_allowOverfull && $this->getConfiguration('offset_nextFullCyle') != '' && $this->getConfiguration('offset_nextFullCyle') > 0 && $this->getCache('last_power',0) >= $this->getConfiguration('threshold_heathot',100)) {
			if($this->getCache('last_power',0) >= 100){
				log::add('thermostat', 'debug', $this->getHumanName() . __(' Cycle précédent à 100%, applique offset : '.$this->getConfiguration('offset_nextFullCyle').'%', __FILE__));
				$power -= $this->getConfiguration('offset_nextFullCyle');
			}else{
				log::add('thermostat', 'debug', $this->getHumanName() . __(' Cycle précédent à '.$this->getCache('last_power',0).'%, applique offset : '.$this->getConfiguration('offset_nextFullCyle').'%', __FILE__));
				log::add('thermostat', 'debug', $this->getHumanName() . __(' Puissance de chauffe du cycle : '.$power.'% - '.$this->getConfiguration('offset_nextFullCyle').'% + '.(100 - $this->getCache('last_power',0)).'%', __FILE__));
	...
	public function getNextState() {
		...
		try {
			...
			log::add('thermostat', 'debug', $this->getHumanName() . __(' Plugin agenda non détecté', __FILE__));
			return '';
		}
		if (!class_exists('calendar_event')) {
			return '';
		}
		log::add('thermostat', 'debug', $this->getHumanName() . __(' Plugin agenda détecté', __FILE__));
		...
		if ($next == null || $next['date'] == '') {
			log::add('thermostat', 'debug', $this->getHumanName() . __(' Smartstart : aucun événement trouvé', __FILE__));
			return '';
		}
	...
	public function postSave() {
		...
		if ($this->getConfiguration('customCmd') != '') {
			...
			$customCmd->setName(__($customCmdName, __FILE__));
			$customCmd->setUnite($customCmdUnite);
			$customCmd->setGeneric_type($customCmdGeneric);
			$customCmd->save();
			if (is_nan($customCmd->execCmd()) || $customCmd->execCmd() == '') {
	...
	public function heat($_repeat = false) {
		...
		log::add('thermostat', 'debug', $this->getHumanName() . __(' Action chauffage', __FILE__));
		...
		foreach ($this->getConfiguration('heating') as $action) {
			try {
				...
				log::add('thermostat', 'error', $this->getHumanName() . __(' Erreur lors de l\'exécution de ', __FILE__) . $action['cmd'] . __('. Détails : ', __FILE__) . $e->getMessage());
			}
		}
		if (!$_repeat) {
			$this->refresh();
	...
	public function cool($_repeat = false) {
		...
		log::add('thermostat', 'debug', $this->getHumanName() . __(' Action froid', __FILE__));
		$consigne = $this->getCmd(null, 'order')->execCmd();
	...
	public function stopThermostat($_repeat = false, $_suspend = false) {
		...
		log::add('thermostat', 'debug', $this->getHumanName() . __(' Action stop', __FILE__));
		$consigne = $this->getCmd(null, 'order')->execCmd();
	...
	public function failure($_failureRepeat = 999) {
		...
		log::add('thermostat', 'debug', $this->getHumanName() . __(' Action défaillance sonde', __FILE__));
		$consigne = $this->getCmd(null, 'order')->execCmd();
	...
	public function failureActuator() {
		...
		log::add('thermostat', 'debug', $this->getHumanName() . __(' Action défaillance chauffage', __FILE__));
		$consigne = $this->getCmd(null, 'order')->execCmd();
...
class thermostatCmd extends cmd {
	...
	public function execute($_options = array()) {
		...
		if ($this->getLogicalId() == 'deltaOrder') {
			...
			return round(jeedom::evaluateExpression($eqLogic->getConfiguration('customCmd')), 1);
