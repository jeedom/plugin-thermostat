...
-   el modo **temporel** calcula un porcentaje de calentamiento en un
...
-   programación completa con el complemento de agenda, incluido en particular el
    posibilidad de anticipar el cambio de consigna para que el
...
-   configuración y rango operativo : chauffage
...
-   La configuración de los modos define temperaturas de
    ...
    empieza a ver aquí las posibilidades de personalización
    plugin.
...
especifique aquí el objeto principal, la activación y la visibilidad del
...
El termostato se utiliza en calefacción, aire acondicionado o ambos.
...
> El ``límite superior de temperatura`` define el rango de
...
la vida Por ejemplo, el modo **Nuit** o **Eco** dar la temperatura que
deseas cuando todos duermen. El modo **Jour** ou
...
**Vacances** programa el termostato a 15 ° C en caso de ausencia prolongada.
...
-   marca la casilla **Visible** para que aparezca este modo en el
...
su aire acondicionado, por ejemplo, para ventilar la habitación para la cual
...
-   ajustar el tiempo después de cerrar la ventana permitiendo
    ...
    ```` Encienda nuevamente si está cerrado por (min) :``
...
Si las sondas utilizadas por el termostato no devuelven ninguna **changement**
...
Cable piloto. Más simplemente enviando un mensaje de texto o un
...
> ``retraso máximo entre 2 lecturas de temperatura``.
...
> ``Margen de falla en caliente`` para calefacción y
> ``Margen de falla en frío`` para aire acondicionado.

En esta imagen, la acción de falla envía la orden para cambiar a
Modo ECO del radiador por el cable piloto, luego envía un mensaje por el
plugin pushbullet.

![Défaillance calefaccion](../images/defaillancechauffage.png)
...
-   **Coeficiente de calentamiento / coeficiente de enfriamiento** : il
...
-   **Autoaprendizaje** : casilla de verificación para activar / desactivar
...
-   **Margen de falla en caliente / Margen de falla en frío** : cette
...
Configuración del plugin. Deberá utilizar el * Resumen de automatización del hogar * (menú
General) para configurarlos. También serán utilizables en
los escenarios.

![Liste des Comandos dans le résumé
domótica](../ images / thermostatlistecommandes.png)
...
vacaciones ... En resumen, controle el termostato de acuerdo con su estilo de vida.
...
Entonces vamos a crear una nueva agenda llamada **Programmation
chauffage**, a lo que agregaremos los eventos de cambio de modo de la
thermostat.
...
En el menú ``Inicio``, está el submenú`` Termostato``. Ventana
...
>    utilizando el termostato para diferentes tipos de calefacción
...
>
> Intente deshabilitar Adbcerradura (o cualquier otro bloqueador de anuncios), por alguna razón desconocida estos bloquean el JavaScript de la página sin motivo.
